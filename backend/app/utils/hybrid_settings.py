...
class HybridSettings(BaseSettings):
    ...
    def __init__(self, **kwargs):
        ...
        self._load_providers_yaml()

    def _load_providers_yaml(self):
        """providers.yaml에서 추가 설정을 로드하여 덮어씁니다."""
        yaml_path = Path(__file__).parent.parent / "config" / "providers.yaml"
        if not yaml_path.exists():
            logger.debug(f"providers.yaml not found at {yaml_path}")
            return
        ...
        try:
            with open(yaml_path, "r", encoding="utf-8") as f:
                ...
                p = data["providers"]
                # Ollama
                if "local_ollama" in p:
                    self.LOCAL_OLLAMA_URL = p["local_ollama"].get("url", self.LOCAL_OLLAMA_URL)
                    self.LOCAL_OLLAMA_MODEL = p["local_ollama"].get("model", self.LOCAL_OLLAMA_MODEL)
                
                # Groq
                if "groq" in p:
                    self.GROQ_MODEL = p["groq"].get("model", self.GROQ_MODEL)
                
                # Claude
                if "claude_haiku" in p:
                    self.ANTHROPIC_MODEL = p["claude_haiku"].get("model", self.ANTHROPIC_MODEL)
                
                # HF
                if "huggingface" in p:
                    self.HUGGINGFACE_MODEL = p["huggingface"].get("model", self.HUGGINGFACE_MODEL)

                logger.info(f"✅ providers.yaml 설정 로드 완료 ({yaml_path})")
        except Exception as e:
            logger.error(f"❌ providers.yaml 로드 실패: {e}")
    ...
    def validate_llm_providers(self) -> dict:
        ...
        # 로컬 Ollama (헬스체크는 부트스트랩에서 수행, 여기서는 설정 기반 상태 표시)
        import httpx
        local_up = False
        try:
            # 동기식 체크 (짧은 타임아웃)
            with httpx.Client(timeout=0.1) as client:
                resp = client.get(self.LOCAL_OLLAMA_URL)
                local_up = resp.status_code == 200
        except Exception:
            pass

        available["local_ollama"] = {
            "status": "available" if local_up else "not_ready",
